...
#' @param ef_fix = Listado de nombres de columnas que son fijos en el archivo 
...
if (!is.null(modelos)) {
  ao = grep(paste(c(paste0("^",modelos,"$")), collapse = "|"), 
            colnames(data))
  
  ef_fix1 = grep(paste(c(colnames(data)[-c(ao)]), collapse = "|"), colnames(data))
  # rm(ao)
} else { #If is.null(modelos)
  ef_fix1 = c(ef_fix,grep(paste(c("SNP","Chromosome","Position"), collapse = "|"),
                          colnames(data)))
}
  # Numero de Traits o corridas del mismo modelo
  n1 <- length(colnames(data)) - length(ef_fix1)
  
  if (n1 == 1) {
    if (is.null(modelos)) {
      ci = colnames(data)[-c(ef_fix1)]
    } else {
      ci = modelos
    }
    as = grep(ci, colnames(data))
    d1 <- data[,c(grep(paste(c("SNP","Chromosome","Position"), collapse = "|"),
                       colnames(data)),as)]
    ...
    d2 <- data[,c(grep(paste(c("SNP","Chromosome","Position"), collapse = "|"),
                       colnames(data)),as)]
...
#       ef_fix = c(2,5,6,7),# c("Dose","Ref","Alt","Trait1","Modelo"), 
#       escala=1, yim = 0.5,xim = 0.02, colores = NULL
#       )
