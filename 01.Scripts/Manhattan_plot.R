...
manhattan.plot_FSA <- function(data,traits=NULL,models=NULL, 
  ...
  render_plot <- function(df, mark) {
    ...
    p <- ggplot(df, aes(x=x, y=Q, colour=factor(Chrom), shape=model)) +
      geom_point() +
      geom_hline(yintercept = -log10(umbral), 
                 col = uc, linewidth = us) +
      {if(!is.null(umb2)) geom_hline(yintercept = -log10(umb2), 
                                     col = uc, 
                                     linewidth = us,
                                     linetype = "dotted")} +
  ...
  if (is.null(models)) {
    ...
    if (!is.null(mkr)) {
      mmmmm = plotme %>% filter(Marker %in% mkr) %>% 
        dplyr::select(c("Marker","Chr","x","qvalue")) %>% 
        mutate(Q = -log10(qvalue),
               Name = paste0("CC-SNP", seq(1,nrow(.))))
    } else {
      mmmmm = NULL
    }
    ...
    for (jkq in 1:n.trait) {
        ...
        for (qwe in 1:n.model) {
          ...
          asd$Q <- -log10(asd$qvalue)
          p <-  render_plot(df = asd, mark = mmmmm)
...
#'@example path.R 
#'Analisis proveniente de GWASpoly
#'Modo de uso ejemplo:

# load("//192.168.153.238/biodata10/rstudio/fernando/02.GWAS/Agronomicas/02.GWASpoly/GWAS_Agronomicas_Individual_ALT_GWASpoly.RData")
#manhattan.plot_FSA(data = MATE,traits = "Todos",
#                         models = c("general","additive"),map = map,
#                         chrom.hold = NULL, solid = TRUE,
#                         FDR = TRUE, umbral = 0.05, umb2 = 0.01, nme = NULL,
#                         plot = F, xtes= 15, hj = 0.5, lp = "none",
#                         at = 15, axt = 14, scx = "Chromosome",
#                         scm = c("#EC5f67", "#FAC863",'#99C794','#6699CC','#C594C5'),
#                         ma = NULL, mi = NULL, us = 1, uc = "darkgreen", NOMBRE = "Contig-Scaffolds)    

#' @example path.R Analisis 
# Analisis proveniente de SOMMER
# PL = manhattan.plot_FSA(data = Sa,traits = "Todos",
#                         models = c("general","additive"),map = map,
#                         chrom.hold = NULL, solid = TRUE,
#                         FDR = TRUE, umbral = 0.05, umb2 = 0.01, nme = NULL,
#                         plot = F, xtes= 15, hj = 0.5, lp = "none",
